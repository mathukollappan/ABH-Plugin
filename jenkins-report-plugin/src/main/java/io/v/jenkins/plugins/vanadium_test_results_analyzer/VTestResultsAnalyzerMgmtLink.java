...
public class VTestResultsAnalyzerMgmtLink extends ManagementLink {
  ...
  public FormValidation doTestConnection(
    ...
    String errMsg = checkConnection(serverIP, userName, rootPassword, dbName);
    if (errMsg.isEmpty()) {
  ...
  public JSONObject checkDatabaseStatus() {
    ...
    String message = checkConnection(plugin.getServerIP(), plugin.getUserName(), plugin.getRootPassword(), plugin.getDbName());
    if (!message.isEmpty()) {
        ret.put(KEY_DB_STATUS_MYSQL_ERR, message);
        return ret;
      }
  ...
  public String fixDatabaseAndTables() {
    ...
    try {
        checkDB = Util.getConnectionForMongo(plugin.getServerIP(), plugin.getUserName(), plugin.getRootPassword(), plugin.getDbName());
    } catch (UnknownHostException ex) {
      checkDB = false;
    }
    if (checkDB ) {
      try {
          MongoClient mongoClient = new MongoClient(plugin.getServerIP(), 27017);
          DB db = mongoClient.getDB(plugin.getDbName());
          char a[] = plugin.getRootPassword().toCharArray();
          boolean auth = db.authenticate(plugin.getUserName(), a);
          if(auth){
  ...
  private String checkConnection(String serverIP, String userName, String rootPassword, String dbName) {
    boolean connection = false;
    try {
      connection = Util.getConnectionForMongo(serverIP, userName, rootPassword, dbName);
    } catch (UnknownHostException e) {
      return e.getMessage();
    }
    if (!connection ) {
